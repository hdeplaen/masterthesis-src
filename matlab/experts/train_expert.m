...
switch type                                                 % for each TYPE
    ...
    case 'par-svm'                                          % TYPE PARALLEL SVM
        ...
        for idx_model = 1:n_classes                         % one-against-all model
            ...
            tot_sv = tot_sv + loc_model.num_sv ;
            models{idx_model} = loc_model ;                 % assign each ls-svm
        end
    case 'par-svm-chi2'                                          % TYPE PARALLEL SVM
        type_svm = params.type ;
        models = cell(n_classes,1) ;                        % prealloc
        for idx_model = 1:n_classes                         % one-against-all model
            load(['chi2/archive_par/chi2_' ...
                num2str(classes{idx_model})],'chi2') ;
            loc_model = train_svm_chi2( ...                      % train each ls-svm apart
                TrainX,TrainY,classes(idx_model),type_svm, chi2) ;
            models{idx_model} = loc_model ;                 % assign each ls-svm
        end
    ...
    case 'seq-svm-chi2'                                          % TYPE SEQUENTIAL SVM
        type_svm = params.type ;
        in_seq = params.seq ;
        [X_out, Y_out] = generate_seq(TrainX,TrainY, in_seq) ;
        for idx_model = 1:n_classes-1                       % one-against-all model
            load(['chi2/archive_seq/chi2_' ...
                num2str(classes{idx_model})],'chi2') ;
            loc_model = train_svm_chi2( ...                      % train each ls-svm apart
                X_out{idx_model}, ...
                Y_out{idx_model}, ...
