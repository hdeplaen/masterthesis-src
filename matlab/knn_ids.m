...
%     Yte_sim(idx) = unique_outputs(idx_best) ;
    Yte_sim(idx) = mean(TrainY(idx_sortdist(1:k))) ;
end

%% RETURN
assert(nargout==1, 'No output arguments needed') ;
varargout{1} = Yte_sim ;

%% TEST
% Yte_sim_bis = sign(Yte_sim) ;
% 
% test_acc = sum(TestY==Yte_sim_bis)/numel(TestY) ;
% test_adder = TestY+Yte_sim_bis ;
% test_tp = sum(test_adder== 2)/numel(TestY) ;
% test_tn = sum(test_adder==-2)/numel(TestY) ;
% test_substr = TestY-Yte_sim_bis ;
